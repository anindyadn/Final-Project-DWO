...
$sql = "SELECT CONCAT('name:\'',d.year,'\'') as account_number, CONCAT('y:', SUM(fs.order_quantity)/(SELECT SUM(order_quantity) AS tot_qty FROM fact_sales)*100) as y , CONCAT('drilldown:', d.year) as drilldown FROM customer p JOIN fact_sales fs ON p.customerID = fs.customerID JOIN date_dimens d ON d.dateID = fs.dateID GROUP BY d.year";
$sql1 = mysqli_query($koneksi,$sql);
while($sql2 = mysqli_fetch_assoc($sql1)){
  $chart1[] = '{'.$sql2['account_number'].','.$sql2['y'].','.$sql2['drilldown'].'},';
}
$sqll = "SELECT SUM(fs.order_quantity)/(SELECT SUM(fs1.order_quantity) AS total_penjualan
FROM customer p1 JOIN fact_sales fs1
ON p1.customerID = fs1.customerID
JOIN date_dimens d1
...
ON d1.dateID = fs1.dateID 
WHERE d1.year = d.year
GROUP BY d1.year) AS total_pertahun , p.account_number, d.year
FROM customer p JOIN fact_sales fs
ON p.customerID = fs.customerID
JOIN date_dimens d
ON d.dateID = fs.dateID
GROUP BY p.account_number, d.year  
ORDER BY `d`.`year`  ASC";
...
while($sqll2 = mysqli_fetch_assoc($sqll1)){
  ...
  $tahun[] = $sqll2['year'];
  $account_number[] = $sqll2['account_number'];
}
for ($i=0; $i < count($tahun) ; $i++) { 
  ...
  $chart22 .= '[`'.$account_number[$i].'`,'.$total_penjualan[$i].'],';
  if ($i ==count($tahun)-1 ) {
   $chart22 .=']}'; 
   break;
  }
