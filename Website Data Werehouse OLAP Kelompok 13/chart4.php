...
$sql = "SELECT CONCAT('name:\'',d.year,'\'') as employee_name, CONCAT('y:', SUM(fs.order_quantity)/(SELECT SUM(order_quantity) AS tot_qty FROM fact_sales)*100) as y , CONCAT('drilldown:', d.year) as drilldown FROM employee p JOIN fact_sales fs ON p.employeeID = fs.employeeID JOIN date_dimens d ON d.dateID = fs.dateID GROUP BY d.year";
$sql1 = mysqli_query($koneksi,$sql);
while($sql2 = mysqli_fetch_assoc($sql1)){
  $chart1[] = '{'.$sql2['employee_name'].','.$sql2['y'].','.$sql2['drilldown'].'},';
}
$sqll = "SELECT SUM(fs.order_quantity)/(SELECT SUM(fs1.order_quantity) AS total_penjualan
FROM employee p1 JOIN fact_sales fs1
ON p1.employeeID = fs1.employeeID
JOIN date_dimens d1
...
ON d1.dateID = fs1.dateID 
WHERE d1.year = d.year
GROUP BY d1.year) AS total_pertahun , p.employee_name, d.year
FROM employee p JOIN fact_sales fs
ON p.employeeID = fs.employeeID
JOIN date_dimens d
ON d.dateID = fs.dateID
GROUP BY p.employee_name, d.year  
ORDER BY `d`.`year`  ASC";
...
while($sqll2 = mysqli_fetch_assoc($sqll1)){
  ...
  $tahun[] = $sqll2['year'];
  $employee_name[] = $sqll2['employee_name'];
}
for ($i=0; $i < count($tahun) ; $i++) { 
  ...
  $chart22 .= '[`'.$employee_name[$i].'`,'.$total_penjualan[$i].'],';
  if ($i ==count($tahun)-1 ) {
   $chart22 .=']}'; 
   break;
  }
