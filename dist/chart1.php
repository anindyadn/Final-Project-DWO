...
$sql = "SELECT CONCAT('name:\'',d.year,'\'') as product_name, CONCAT('y:', SUM(fs.order_quantity)/(SELECT SUM(order_quantity) AS tot_qty FROM fact_sales)*100) as y , CONCAT('drilldown:', d.year) as drilldown FROM product p JOIN fact_sales fs ON p.productID = fs.productID JOIN date_dimens d ON d.dateID = fs.dateID GROUP BY d.year";
